...
//    private boolean mIsDisplayTag;                                      // 是否显示标签
    ...
    protected Paint mCheckInPaint;
    protected Paint mCheckOutPaint;
    ...
    protected int mCheckInTextColor;                     // 入住标签字体颜色
    protected int mCheckOutTextColor;                  // 退房标签字体颜色
...
//        mDrawRect = typedArray.getBoolean(R.styleable.DayPickerView_drawRoundRect, true);
        mCheckInTextColor = typedArray.getColor(R.styleable.DayPickerView_colorCheckInText, resources.getColor(R.color.normal_day));
        mCheckOutTextColor = typedArray.getColor(R.styleable.DayPickerView_colorCheckOutText, resources.getColor(R.color.normal_day));
    ...
    private void drawMonthDayLabels(Canvas canvas) {
        ...
        canvas.drawLine(0, y + WEEK_TEXT_SIZE / 2, mWidth, y + WEEK_TEXT_SIZE / 2, paint);
    }
    ...
    private void drawMonthTitle(Canvas canvas) {
        ...
        canvas.drawRect(0, 0, mWidth, y + (WEEK_TEXT_SIZE * 3 / 4), mTitleBGPaint);
    ...
    protected void drawMonthCell(Canvas canvas) {
        ...
        while (day <= mNumCells) {
            ...
            if (mHasToday && (mToday == day)) {
                isToady = true;
                canvas.drawText("今天", x, y - DAY_SELECTED_RECT_SIZE /2, mCheckInPaint);
            }
            // 已过去的日期
            ...
            if (mStartDate != null && cellCalendar.equals(mStartDate) && !mStartDate.equals(mEndDate)) {
                ...
                canvas.drawText("入住", x, getTextYCenter(mDayTextPaint, y + DAY_SELECTED_RECT_SIZE / 2), mCheckInPaint);
//                if (isToady) {
//                    canvas.drawText("今天", x, getTextYCenter(mDayTextPaint, y - DAY_SELECTED_RECT_SIZE / 2), mCheckInPaint);
//                }
            }
            ...
            if (mEndDate != null && cellCalendar.equals(mEndDate) && !mStartDate.equals(mEndDate)) {
                ...
                drawDayBg(canvas, x, y, mSelectedDayBgPaint);
                mDayTextPaint.setColor(mSelectedDayTextColor);
                canvas.drawText("退房", x, getTextYCenter(mDayTextPaint, y + DAY_SELECTED_RECT_SIZE / 2), mCheckOutPaint);
            }
...
//                            canvas.drawRoundRect(rectF, 10.0f, 10.0f, mBusyDayBgPaint);
                            ...
                            mTagTextPaint.setColor(mBusyDaysTextColor);
                        }
                        canvas.drawText("已租", x, getTextYCenter(mBusyDayBgPaint, y + DAY_SELECTED_RECT_SIZE / 2), mTagTextPaint);
                    }
...
//                            canvas.drawRoundRect(rectF, 10.0f, 10.0f, mBusyDayBgPaint);
                            ...
                            mTagTextPaint.setColor(mInValidDaysTextColor);
                        }
                        canvas.drawText("禁用", x, getTextYCenter(mInValidDayBgPaint, y + DAY_SELECTED_RECT_SIZE / 2), mTagTextPaint);
                    }
                    canvas.drawText(String.format("%d", day), x, getTextYCenter(mTagTextPaint, y ), mDayTextPaint);
                }
            }
...
//                    canvas.drawRoundRect(rectF, 10.0f, 10.0f, mBusyDayBgPaint);
            ...
            // 绘制日期
            if ( !isPrevDay && !isInvalidDays && !isBusyDay) {
                canvas.drawText(String.format("%d", day), x, getTextYCenter(mTagTextPaint, y), mDayTextPaint);
            }
    ...
    protected void initView() {
        ...
        mTitleBGPaint.setColor(Color.parseColor("#EFEFEF"));
...
//        mSelectedDayTextPaint.setFakeBoldText(true);
        ...
        // 入住标签字体paint
        mCheckInPaint = new Paint();
        mCheckInPaint.setAntiAlias(true);
        mCheckInPaint.setColor(mCheckInTextColor);
        mCheckInPaint.setTextSize(TAG_TEXT_SIZE);
        mCheckInPaint.setStyle(Style.FILL);
        mCheckInPaint.setTextAlign(Align.CENTER);
        mCheckInPaint.setFakeBoldText(false);

        // 退房标签字体paint
        mCheckOutPaint = new Paint();
        mCheckOutPaint.setAntiAlias(true);
        mCheckOutPaint.setColor(mCheckOutTextColor);
        mCheckOutPaint.setTextSize(TAG_TEXT_SIZE);
        mCheckOutPaint.setStyle(Style.FILL);
        mCheckOutPaint.setTextAlign(Align.CENTER);
        mCheckOutPaint.setFakeBoldText(false);
    }
